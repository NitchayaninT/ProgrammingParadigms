...
class w10_2_SemanticEvent extends JFrame
{
	...
	// ----- (1) anonymous listener classes (implicitly implement listeners)
        String local = "local";
        final String finlocal = "final local"; //explicitly final
        
        add_button = new JButton("Add");
        add_button.addActionListener( new ActionListener() { //for add button
            @Override
            public void actionPerformed( ActionEvent e )
            {
                ...
                drawpane.doAdd(); //drawpane = panel (Container of many objects)
            }
        });
	...
	rem_button.addActionListener( new ActionListener() { //for remove button
            @Override
            public void actionPerformed( ActionEvent e )
            {
		drawpane.doRemove();
...
class MyPane extends JPanel
{
    ...
    public void doAdd()//called when the user clicks Add
    {
    ...
    public void doRemove()
    {
        for (int i = 0; i < count; i++)
        {
            ...
            {
                Component clabel = getComponent(0); //remove the first one
                remove( clabel );
